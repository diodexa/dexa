...
const Halaman7  = ({data,isActive}:Props) =>  {
  return (
    ...
    style={{background: data.theme?.warna1, color: data.theme?.warna2}}>
        {data.Background?.Background7 ? (
        <img src={data.Background.Background7} alt="" className="absolute inset-0 w-full h-full object-cover" />
        ) : <>
            ...
            <div className="flex flex-col gap-1  items-end p-2" >
                ...
                (
                    <div className="w-full flex flex-col items-end">
                        ...
                        <a href={data.LinkGoogleMapsResepsi} target="blank" style={{pointerEvents: isActive ? "auto" : "none"}}>
                            <button className="border-1 text-xs flex p-1 items-center rounded-md hover:scale-110 transition" style={{background:data.theme?.warnaButtonBackground, color:data.theme?.contrasfont}}>
                                <svg
                                xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 24 24"
                                className="w-6 h-6"
