...
const Halaman8  = ({data}:Props) =>  {
  return (
    ...
    style={{background: data.theme?.warna1, color: data.theme?.warna2}}>
      ...
      <div className="flex flex-col items-end  w-full h-full gap-2 pt-5">
          ...
          <div className="flex flex-col gap-1 items-start justify-center p-2 ">
            {data.Story?.slice(0,2).map((Cerita,index)=>{
              return (
                <div key={index} className={`flex flex-row-reverse justify-between border p-2 w-full  mb-1 `}>
                  <div className="leading-none text-right w-2/4 pr-1">
                      <h2 className="text-[0.7rem] lg:text-[0.6rem] mb-1"><strong>{Cerita.Head}</strong></h2>
                      <p className="text-[0.5rem] lg:text-[0.5rem] whitespace-pre-line">{Cerita.Story}</p>
                  </div>
