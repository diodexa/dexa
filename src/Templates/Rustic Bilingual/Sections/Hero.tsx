  ...
  return (
    <section className={`fixed inset-0 z-9998 mx-auto flex max-w-[385px] items-center justify-center overflow-hidden transition-all duration-800 ${HideHero ? "pointer-events-none opacity-0" : "opacity-100"}`} style={{ background: data.theme?.warnaweddingInvitation, color: data.theme?.warna3 }}>
      <div className={`relative flex h-[520px] w-[330px] items-end justify-center `}>
        ...
        {/* KARTU UNDANGAN */}
        <div className={`absolute bottom-[80px]  h-[240px] w-[300px] flex flex-col items-center justify-start border bg-white px-6 pt-8 text-center shadow-xl  [transform-origin:center_bottom] ${isOpen ? "BukaKartu" : " "}`} style={{ background: data.theme?.warna1, borderColor: `${data.theme?.warna3}60`, color: data.theme?.warna3 }}>
            <p className="text-[8px] uppercase tracking-[.45em] opacity-60">The Wedding Of</p>
            <div className="flex items-center justify-center gap-3">
                <p className="text-2xl font-Tempting" style={{ color: data.theme?.warna2 }}>{data.NamabridePanggilan}</p>
                <p className="my-1 text-lg">&</p>
                <p className="text-2xl font-Tempting" style={{ color: data.theme?.warna2 }}>{data.NamagroomPanggilan}</p>
            </div>
        ...
        {/* GUEST */}
        <div className={`absolute -bottom-1  text-center transition-all duration-500 ${isOpen ? "opacity-0" : "opacity-100"}`} style={{color: data.theme?.warna1}}>
